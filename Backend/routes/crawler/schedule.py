...
def start_scheduler(flask_app: Flask):
    ...
    if scheduler is None: # 避免重複啟動排程器，只啟動一次。
        try:
            ...
            scheduler.add_job(scheduled_task, 'interval', minutes=10) # 設定排程器，每 10 分鐘執行一次

            tz = pytz.timezone('Asia/Taipei')
            next_run = datetime.now(tz) + timedelta(minutes=10) #  記錄「下一次執行時間」變數，供前端查詢用

            scheduler.start()# 啟動排程器

            scheduled_task() # 立即執行一次，啟動後不用等 10 分鐘
...
def scheduled_task():
    ...
    try: # Flask 的資料庫操作需要有「應用上下文」，這句是必要的包裝！
        with app.app_context():
            ...
            tz = pytz.timezone('Asia/Taipei')
            now = datetime.now(tz)
            future = now + timedelta(minutes=10) # 記錄上次與下次執行時間（給前端 info 顯示）
